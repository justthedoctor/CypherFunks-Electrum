...
from electrum.util import (parse_URI, InvalidCypherfunksURI, TxMinedInfo, maybe_extract_bolt11_invoice,
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Cypherfunks address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Cypherfunks Address') + ':\n' + address)
                        return
